...
If we are building segment tree on interval $[1, n]$ with root index 1 over an array ($2x$ and $2x + 1$ are children of $x$) and if $\mathit{IDX}[n]$ is maximum index of a node in segment tree representing a valid interval; then it can be shown (much more easily observed emperically) that $\frac{\mathit{IDX}[n]}{n}$ is maximum when $n = 2^k + 2^{\lfloor\frac{k}{2}\rfloor}$ for some $k \ge 1$ (and also $\mathit{IDX}[n] = 2^{k+2} - 2^{\lfloor\frac{k + 1}{2}\rfloor + 1} + 1$).

And on limit, $\lim_{k \to \infty} \frac{2^{k+2} - 2^{\lfloor\frac{k + 1}{2}\rfloor + 1} + 1}{2^k + 2^{\lfloor\frac{k}{2}\rfloor}} = 4$.
